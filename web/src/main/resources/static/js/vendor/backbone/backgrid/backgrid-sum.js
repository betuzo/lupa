  ...
  var SummedColumnBody = window.Backgrid.SummedColumnBody = window.Backgrid.Body.extend({
    ...
    getFormatterForColumn: function (column) {
      if (this.formatters && this.formatters[column.get('name')]) {
        ...
        return new window.Backgrid.NumberFormatter();
      }
    },
    ...
    getSumRow: function () {
      ...
      return new (
        Backbone.View.extend({
          ...
          tagName: 'tr',
          render: function () {
            _(_this.columns.models).each(function (column) {
              var sum = '';
              ...
              this.$el.append(_this.template({ className: 'renderable number-cell', sum: sum }));
            }, this);
